...
export default function TexnikaRepair() {
  ...
  const handleSendToUnused = async (user) => {
    try {
      const token = localStorage.getItem("token");
      const username = localStorage.getItem("username"); // Get the username
      
      // If username exists, add it to the user object
      if (username) {
  ...
  const handleModalTexnika = async (e) => {
    ...
    try {
      ...
      if (response.status === 403) {
        ...
        // Xabar ko'rsatilgandan so'ng 3 soniya ichida login sahifasiga yo'naltirish
        setTimeout(() => {
      ...
      if (response.ok) {
        ...
        const errorResult = await response.json();
        setSuccess(false);
        setText(errorResult.message || "Texnika yangilanishida xatolik yuz berdi");
      }
    } catch (error) {
      setSuccess(false);
      setText(error.message || "Texnika yangilanishida xatolik yuz berdi");
    } finally {
      setShowSuccess(true);
      setTimeout(() => {
        setShowSuccess(false);
