...
export default function History({ closeModal, user }) {
  ...
  const handleDownload = async (filePath) => {
    ...
    const url = `${BACK_API}api/download/${encodeURIComponent(filePath)}`;
    const token = localStorage.getItem("token");
            ...
            {historyData.length > 0 ? (
              <thead>
                <tr>
                  ...
                  <th>Статус</th>
                  <th>Сотрудник имя</th>
                  <th className="short-column">Сотрудник подрозделений</th>
                  <th className="word-column">файл</th>
                  <th>Дата</th>
                </tr>
              </thead>
            ) : (
            ...
            <tbody>
              {historyData.map((row, index) => (
                <tr key={row.id}>
                  ...
                  <td>{row.employee_podrazdelenie}</td>
                  <td>
                    {row.word_file_path && (
                      <button
                        onClick={() => handleDownload(row.word_file_path)}
                        className="history_info-table-download"
                      >
                        <FaDownload />
                      </button>
                    )}
